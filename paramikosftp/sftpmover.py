...
def transfer_to_this_remote_dir():
    """ask user for full path of dir to be created"""
    while True:
        what_dir = input("What directory on remote hose would you like to move files to? ")
        if what_dir == "":
            what_dir = "/tmp/"
            break
...
def main():
   ...
   #call the function that moves files
    movethemfiles(sftp)
